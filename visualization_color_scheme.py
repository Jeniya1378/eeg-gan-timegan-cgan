...
def cluster_id(posture, cond, domain, color_scheme="36"):
    ...
    if color_scheme == "36":
        ...
        # 🔹 Force posture 2 to be in the low indices (darker colors)
        if posture == 2:
            idx = c*2 + (0 if d==1 else 1)  # always within 0..3 → dark colors
        return idx
...
def save_legend(ax, out_path, ncols=3, fontsize=5.5):
    """Save the legend from an existing Axes to a standalone image."""
    # Build the legend from current handles/labels
    handles, labels = ax.get_legend_handles_labels()
    fig_leg = plt.figure(figsize=(8, 2))  # adjust size to your needs
...
def main():
    ...
    ap.add_argument("--out",       type=str, default="./eval_out_plots_v2")
