...
Per posture, the model is conditioned on {no_exo=0, with_exo=1}.
...
def discriminative_metrics(X_real, X_gen, y_posture, out_csv):
    ...
    for p in posts:
        mr = (yr==p); mg = (yg==p)
        Xp = np.vstack([Xs[:len(Fr)][mr], Xs[len(Fr):][mg]])
        yp = np.hstack([y[:len(Fr)][mr], y[len(Fr):][mg]])
...
def predictive_scores(X_real, X_gen, y_posture, out_csv, ch_names=None):
    # target channel
    if TARGET_CHANNEL and ch_names is not None and TARGET_CHANNEL in list(ch_names):
        target_idx = list(ch_names).index(TARGET_CHANNEL)
    else:
        target_idx = C-1
    ...
    for p in posts:
        mr = (yr == p); mg = (yg == p)
        if mr.sum() < 5 or mg.sum() < 5: continue
        # TSTR
        sX, sY = StandardScaler(), StandardScaler()
...
# ---- PCA/t-SNE (fixed label sizes) ----
def scatter_plots(X_real, X_gen, y_posture, out_dir):
    Fr, Fg = psd_features(X_real), psd_features(X_gen)
    X = np.vstack([Fr, Fg])
    n_real = len(Fr)
    assert len(y_posture) == n_real + len(Fg)
    y = y_posture  # already [real; gen]
    ...
    try:
        ...
        tsne = TSNE(n_components=2, perplexity=TSNE_PERPLEXITY, learning_rate=200.0,
...
def main():
    ...
    for p in POSTURES:
        ...
        # collect for global
        Xr_glob.append(R); Xg_glob.append(G); yp_glob.append(yp)
