...
- Global + Local Discriminators (spectral norm, projection + ACGAN)
- Hinge + R1(real), TTUR (G lr > D lr)
- DiffAugment-1D, instance noise, Feature Matching, EMA
- Random-pair coherence loss (+ PSD/Cov), Amplitude calibration (mean/std)
- Pre-warm first epochs for G (no adversarial)
- AMP for D only; G step in fp32
"""

import os, json, csv, time, argparse, itertools
...
def get_args():
    ...
    p.add_argument("--amp-weight", type=float, default=0.5)   # mean/std calibration
    ...
    # control which posture(s) to train
...
def train_one_posture(args, posture:int):
    ...
    for ep in range(args.prewarm + args.epochs):
        prewarm = (ep < args.prewarm)
        t = (ep-0) / max(1, args.prewarm+args.epochs-1)
        ...
        if (ep+1) % 10 == 0 or ep == 0:
            print(f"[posture {posture}] ep {ep+1}/{args.prewarm+args.epochs} "
