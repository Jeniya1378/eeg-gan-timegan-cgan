...
# -*- coding: utf-8 -*-
"""
Combined PCA & t-SNE with a *paired legend layout*:
- Legend shows 18 rows × 2 columns:
    Col 1: Real     (Posture-i <Cond> Real)
    Col 2: Synthetic(Posture-i <Cond> Synthetic)
  in the order of Posture 1..9, and for each posture: No exo, With exo.
- Color scheme:
...
- Posture-2 is biased to darker color indices in the 36-color scheme.

Run example:
  python visualization_color_scheme_paired_legend.py \      --real_dir ./preprocessed \      --synth_dir ./timegan_runs \      --out ./eval_out_plots
"""
...
def cluster_id(posture, cond, domain, color_scheme="36"):
    ...
    if color_scheme == "36":
        ...
        idx = p*2 + c  # 0..17
        # If you'd like posture-2 bias in the 18-color mode too, uncomment below:
        # if posture == 2:
        #     idx = c
        return idx
        ...
        for c in ["with_exo", "no_exo"]:
            for d in [1, 0]:
                ...
                ax.scatter(
                    ...
                    c=[colors[gid]],
                    label=None,  # legend will use proxy artists
                )

def add_paired_legend(ax, colors, color_scheme="36", fontsize=5.6):
    """Add a 2-column legend where each row is (Real, Synthetic) for a posture×condition.
    Rows are ordered by posture 1..9, and within each posture: No exo, With exo.
    """
    handles = []
    labels = []
    for p in range(1, 10):
        for cond in ["no_exo", "with_exo"]:  # user requested 'No exo' first
            # Real (o)
            gid_r = cluster_id(p, cond, 1, color_scheme=color_scheme)
            color_r = colors[gid_r]
            handles.append(Line2D([], [], linestyle='None', marker='o', markersize=5, color=color_r))
            labels.append(f"P-{p} {'No exo' if cond=='no_exo' else 'With exo'} Real")
            # Synthetic (x)
            gid_g = cluster_id(p, cond, 0, color_scheme=color_scheme)
            color_g = colors[gid_g]
            handles.append(Line2D([], [], linestyle='None', marker='x', markersize=5, color=color_g))
            labels.append(f"P-{p} {'No exo' if cond=='no_exo' else 'With exo'} Syn")
...
def main():
    ...
    ax.set_title("PCA: Posture×Condition (● Real, × Gen)")
    ...
    ax.set_title("t-SNE: Posture×Condition (● Real, × Gen)")
    ax.set_xlabel(""); ax.set_ylabel("")
    ...
    print(f"Saved PCA and t-SNE with paired legends to {out_dir}")
