...
Preserves the exact color mapping of the global figure.

Run (no flags needed):
  python visualization_zoom_fixed.py --real_dir ./preprocessed --synth_dir ./timegan_runs --out ./eval_out_plots
"""
        ...
        for c in ["with_exo","no_exo"]:
            for d in [1,0]:
                ...
                ax.scatter(emb[m,0], emb[m,1], s=9, alpha=0.8,
                           marker=('o' if d==1 else 'x'), c=[colors[gid]], label=label)

def add_compact_legend(ax, ncols=3, fontsize=5.2):
...
def main():
    ...
    ax.set_xlabel("Dim 1"); ax.set_ylabel("Dim 2")
    fig.subplots_adjust(right=0.78); add_compact_legend(ax, ncols=3, fontsize=5.2)
    ...
    # Per-(posture×condition) zooms (always on)
    saved_any = False
    for p in range(1, 10):
        for cond in ["with_exo","no_exo"]:
